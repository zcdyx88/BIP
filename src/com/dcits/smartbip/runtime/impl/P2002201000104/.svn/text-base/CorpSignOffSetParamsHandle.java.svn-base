 ...
 *
 */
public class CorpSignOffSetParamsHandle extends AbstractBaseService implements IService {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 5217177234964825802L;
	/**
	 * 日志
	 */
	private static final Log log = LogFactory.getLog(CorpSignOffSetParamsHandle.class);
	

	@Override
	public String getId() {
		return "CorpSignOffSetParamsHandle";
	}
	...
	public Object execute(Object req) throws InvokeException {
		...
		if (log.isInfoEnabled()) {
			log.info("开始调用基础服务[CorpSignOffSetParamsHandle]......");
		}
		...
		try {
			...
			String ReqAcctNo = CompositeDataUtils.getValue(Req2002201000104, "ReqAppBody/AcctNo");//上送解约帐号
			String AcctSrlNo = CompositeDataUtils.getValue(Req2002201000104, "ReqAppBody/AcctSrlNo");//上送账户序号
			String AcctTp = CompositeDataUtils.getValue(Req2002201000104, "ReqAppBody/AcctTp");//账户类型
			if("A".equals(AcctTp)){
			...
			log.debug("对公综合解约ECIF维护账号为["+getContext().getValue("ECIF_AcctNo")+"]");
			log.debug("对公综合解约上送后台系统账号为["+getContext().getValue("CorpComBinationAcctNo")+"]");
		} catch (Exception e) {
			log.error("对公综合解约参数设置处理:",e);
			throw new InvokeException();
		}finally{
			if (log.isInfoEnabled()) {
				log.info("调用基础服务结束[CorpSignOffSetParamsHandle]......");
			}
		}
		return null;
	}
