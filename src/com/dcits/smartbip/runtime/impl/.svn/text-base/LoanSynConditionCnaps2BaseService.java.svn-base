 ...
 * 
 */
public class LoanSynConditionCnaps2BaseService extends AbstractBaseService
		implements IService {
	private static final Log log = LogFactory
			.getLog(LoanSynConditionCnaps2BaseService.class);
	private static final long serialVersionUID = 1L;

	@Override
	public String getId() {
		return "LoanSynConditionCnaps2BaseService";
	}
	...
	public Object execute(Object req) throws InvokeException {
		...
		if (log.isInfoEnabled()) {
			log.info("开始调用基础服务[LoanSynConditionCnaps2BaseService]......");
		}
		//获取贷款类型
		String LoanService = "";
		LoanService =  (String) getContext().getValue("__LOAN_KIND__");
		 ...
		 if(log.isDebugEnabled()){
			 log.debug("核心执行标志[" + ncbsFlag  + "],放款标志[" + makeLoansFlag + "]取3002100000102服务返回");
		 }
		 if("1".equals(ncbsFlag)){
			ICompositeData RespLoanService = (ICompositeData) getContext().getValue("Rsp3002100000102");
			String RetCode = CompositeDataUtils.getValue(RespLoanService, "RspSysHead/RetCode");
			 //核心执行成功，限额不成功。不会到核心。所以要判断限额的。，放款标志是1
			 if("1".equalsIgnoreCase(makeLoansFlag) && "000000".equalsIgnoreCase(RetCode)){
				...
				if (null == Req3004100001820)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
				 ...
				 //如果需要到二代，对金额进行格式化,只有行外帐号才会取帐号，取单笔行外账户到二代,目前数组中能到二代的受托支付数组只能有一个，>=2 手工支付
				...
				//其中Payer_Actno付款人账号  Payer_Name付款人名称取的核心返回的lstdkstzf 这个list的zjzrzhao  dfzhangh
				...
				for (ICompositeData aczNcbsFldComposite : aczNcbsList) {
							CompositeDataUtils.copy(aczNcbsFldComposite, Req3004100001820,"zjlyzhao", "ReqAppBody/Payer_Actno");
							CompositeDataUtils.copy(aczNcbsFldComposite, Req3004100001820,"zjlyzhmc", "ReqAppBody/Payer_Name");
					 }
			 }
		 }else{
			 if(log.isDebugEnabled()){
		...
		if (log.isInfoEnabled()) {
			...
			log.info("调用基础服务结束[LoanSynConditionCnaps2BaseService]......");
		}
		return null;
